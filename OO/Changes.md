...
## 2026-08-18 — 武器军需官上架荣誉奖章（战场牌子），支持花金币购买

**背景**：玩家反馈现在人少，战场基本打不起来，导致荣誉武器兑换所需的"战场牌子"没法正常获取（荣誉点本身可以走其他途径攒）。建议给武器军需官NPC上架4个战场牌子，花金币直接买，作为人少情况下的追赶通道。

**涉及NPC**（军需官，联盟"奥尼尔上尉"/部落"雷角中士"，各3个等级）：12782、24671、26394（联盟）、14581、24667、26396（部落）。

**关键澄清**：玩家最初给的 `currency=121/122/123/125` 对应不上任何数据——那是巫妖王之怒才引入的"货币"系统（CurrencyTypes.dbc）编号，Nmangos-tbc 没有实现货币系统（代码里搜不到），这几个数字在本库里查不到。这4个牌子在本库实际是**普通物品**：
- 奥特兰克山谷荣誉奖章 = 物品20560
- 阿拉希盆地荣誉奖章 = 物品20559
- 风暴之眼荣誉奖章 = 物品29024
- 战歌峡谷荣誉奖章 = 物品20558

**核实**：查了这6个NPC，共用4套 `npc_vendor_template`（不是各自独立配置）：546（联盟低级）、547（部落低级）、553（联盟/部落中级共用）、563（联盟/部落高级共用）——加这4套模板就覆盖全部6个NPC。4个牌子物品原本 `BuyPrice=0`（从没设计成能直接买），也没有声望/荣誉等级门槛。

**改动**（只改了 `tbcmangosdev`，未动 `tbcmangos2`）：
1. `UPDATE item_template SET BuyPrice=10000 WHERE entry IN (20558,20559,20560,29024);`（10000铜=1金）
2. `npc_vendor_template` 里给 546/547/553/563 四套模板各加4行（16行），`ExtendedCost=0`（纯花钱买），`maxcount=0`（不限购）

**不影响的部分**：武器本身的兑换需求（荣誉点数+牌子数量的 ExtendedCost 配置）完全没动，只是给牌子本身多加了一条"花金币买"的获取途径，不绕过荣誉点要求。

**待确认/待决策**（未替用户决定，先按无限购实施）：
...
**背景**：牌子能花钱买了之后，玩家测试发现买武器本身时服务器仍返回"荣誉点不够"——因为武器的兑换需求（`ExtendedCost`）本来就是"荣誉点+牌子"两者都要，只解决牌子的获取途径还不够。用户要求：这几个军需官卖的装备，**只需要牌子，不需要荣誉点**。

**方案选择**：`ExtendedCost` 的荣誉点/牌子数量配置存在 `ItemExtendedCost.dbc`（二进制DBC，不在MySQL里），一开始考虑直接改DBC文件（写了个 `patch_itemextendedcost.py` 脚本清零 `reqhonorpoints` 字段），但DBC文件在远程服务器上、本地访问不到，而且服务器和客户端各有一份、两边不一致会导致界面显示对不上。**改用C++代码层面直接跳过**：不动DBC数据本身，只在服务端购买校验/扣费逻辑里，针对这几个已知的 ExtendedCost 编号跳过荣誉点检查和扣除。DBC脚本作废，不需要使用。

**涉及的 ExtendedCost 编号**（这6个军需官/4套 `npc_vendor_template`546/547/553/563 用到的全部编号）：131、348、2237、2238、2239、2240、2241、2242、2271。

**改动**：`src/game/Entities/Player.cpp`
1. 新增文件级静态函数 `IsHonorlessQuartermasterExtendedCost(uint32 extendedCostId)`，用 switch-case 硬编码判断是否属于上述9个编号
2. `Player::BuyItemFromVendor()` 购买前校验：荣誉点数不足的判断加上 `!IsHonorlessQuartermasterExtendedCost(extendedCostId) &&` 前置条件，命中这9个编号时直接跳过荣誉点校验
3. `Player::TakeExtendedCost()` 购买后扣费：荣誉点扣除同样加上这个判断，命中时不扣荣誉点

**范围**：只精确影响这9个 ExtendedCost 编号（也就是这6个军需官卖的东西），不影响游戏里其他任何用到荣誉点/竞技场点数的购买（其他PVP装备、竞技场军需官等完全不受影响）。牌子（`reqitem`/`reqitemcount`）要求完全没动，武器本身依然要交够数量的牌子才能买。DBC数据本身不做任何修改，客户端界面显示会依然显示"需要XX荣誉点"（纯文字不一致，不影响实际购买判定，因为真正校验走的是服务器代码而不是看客户端显示）。

**是否数据库/代码改动**：代码改动（`.cpp`），需要重新编译 `mangosd` 才能生效，不涉及数据库。

**状态**：✅ 代码已改，**尚未编译部署**，需要重新编译 `mangosd` 并重启才能生效。生效后可以在这6个军需官处测试：只有牌子够、荣誉点为0时也应该能正常买到武器。

## 2026-08-18（修正）— 补上"只影响这6个NPC"的校验，避免误伤共用同一ExtendedCost编号的其他武器商

**问题**：上面的实现只按 `extendedCostId` 判断要不要跳过荣誉点，用户追问"这个只影响这几个npc卖的东西对吧"，核查后发现**不对**——ExtendedCost 编号 `131` 同时也被两个完全无关的NPC共用：`12784`"Jackspring中尉"（卖大元帅系列武器）、`12794`"扎格石卫"（卖高阶督军系列武器），这两个是老版本PVP军衔武器商，跟这次要改的6个军需官毫无关系。原实现会连带让这两个NPC的武器也免荣誉点，属于误伤，不符合用户要求的范围。

**修正**：把判断函数改名为 `IsHonorlessQuartermasterPurchase(vendorEntry, extendedCostId)`，**同时**校验"卖家是不是这6个军需官之一"和"ExtendedCost是不是这9个编号之一"，两个条件都满足才跳过荣誉点。`Player::TakeExtendedCost()` 签名加了第三个参数 `vendorEntry`（默认0，向后兼容——不过全代码库搜索确认这个函数只有这2处调用，都已经改成传入 `pCreature->GetEntry()`），`Player.h` 声明同步更新。
...
## 2026-09-10 — 修复`CalculateSpellEffectValue`对`TRIGGER_SPELL_WITH_VALUE`外部传值的二次缩放bug

**背景**：玩家反馈刀锋山恐惧魔（npc=22204）"邪能烈焰"伤害数值过高，战斗日志显示同一个技能名下伤害忽高忽低——大部分命中在790左右，偶尔一次高达4461（约5.5倍）。

**排查过程**：恐惧魔通过`creature_ai_scripts`释放spell=36248（Fel Flames），这个技能有两条并行路径都指向真正的伤害技能spell=36820：
1. 效果1（`SPELL_EFFECT_TRIGGER_SPELL_WITH_VALUE`）：一次性触发，把自己算好的数值强制塞给36820
2. 效果2（周期光环）：每1秒重新触发一次36820，走36820自己的正常计算路径

36248和36820**两个技能本身都单独带`SPELL_ATTR_SCALES_WITH_CREATURE_LEVEL`缩放属性**（`spellLevel=20`，恐惧魔实际71-72级，等级差超过30级，触发之前系统性修复过的缩放修正公式）。

**根因**：`Spell::EffectTriggerSpellWithValue`（`SpellEffects.cpp:3212-3213`）把36248自己算出的、**已经缩放修正过**的`damage`值，通过`CastCustomSpell(..., &bp, &bp, &bp, ...)`强制塞给36820当基础值。但`CalculateSpellEffectValue`（`Object.cpp`）不区分"这个基础值是外部强制传入的最终值"还是"技能自己模板里的原始配置值"——36820接到这个已经放大过一次的数值后，因为自己也带同样的缩放属性，**又重新按等级差再放大了一次**，变成双重缩放。

用实际数据核算：140(原始值) × 5.548(缩放比例，71级/20级战士血统曲线比) ≈ 777，跟周期跳伤害的790基本吻合；777再被36820二次缩放：777 × 5.548 ≈ 4310，跟异常的4461基本吻合（误差来自恐惧魔实际等级71/72的随机浮动）。

**修复方案的一次重要纠正**：最初提的方案是给缩放修正条件加`&& !effBasePoints`，复查时发现这个思路是**错的**——`Spell::CalculateSpellEffectValue`（`Spell.h:524-525`，几乎所有正常技能伤害计算，包括之前系统性修复的204个技能，都走这条路径）永远传入`&m_currentBasePoints[i]`，非空是常态，不是"外部强制传值"的专属信号，这个条件会让缩放修正对几乎所有正常伤害技能失效。**已废弃**，改用更精确的方案：只在`Effect[effect_index]==SPELL_EFFECT_TRIGGER_SPELL_WITH_VALUE`这一种效果类型上，额外查一下它要触发的目标技能（`EffectTriggerSpell[effect_index]`）是否也带`SPELL_ATTR_SCALES_WITH_CREATURE_LEVEL`——如果是，让**这个中转效果本身**不缩放，把缩放完全交给被触发的技能去做一次。不改动`effBasePoints`判断，不影响任何其它路径。

用SQL查了全库`spell_template`（三个效果槽位都查了），当前**只有36248→36820这一对技能**命中"中转技能和被触发技能都带缩放属性"的组合，改动实际生效范围就是这一个技能链；逻辑本身是通用防护，不是针对这一个技能硬编码，以后出现类似组合也能自动挡住。

**数值同时对齐了原版强度**：查到Wowpedia记录邪能烈焰是"每秒1200-1350点火焰伤害"（[参考](https://wowpedia.fandom.com/wiki/Fear_Fiend)），比双重缩放修好之后单纯只解决bug算出来的约777还要高不少——说明这不只是双重缩放的问题，数值本身也偏低。反推缩放前的骰子基础值区间应为216~243（216×5.548≈1198，243×5.548≈1348），把`spell_template`里36248和36820的`EffectBasePoints1`/`EffectBaseDice1`/`EffectDieSides1`从固定值139(+1偏移)改成`216/0/27`的随机骰子区间，让每次命中在1200-1350码内独立随机浮动（触发路径和周期跳伤害路径各自独立掷骰），而不是钉死一个数字。改之前中间试过一版固定值229（对应≈1276），后来根据"是否可以调整为动态区间"的反馈换成了骰子区间。

修改的两个技能只有恐惧魔（entry=22204）在用（`creature_ai_scripts`核实过，没有其它怪物引用；另发现一个孤立的`creature_spell_list`行id=2220405和一个同名但无关的独立技能34072，两者均未被任何在跑的怪物实际使用，不受影响）。

**是否数据库/代码改动**：两者都有。
- 代码：`src/game/Entities/Object.cpp`（防二次缩放逻辑），需要重新编译部署。
