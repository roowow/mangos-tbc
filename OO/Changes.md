...
**修复**：`Object.cpp`的`CalculateSpellEffectValue`，触发缩放修正的判断条件加一个`&& !effBasePoints`——当基础值是外部强制指定的（比如`CastCustomSpell`传入），跳过缩放修正，只让"最初算出这个数值的技能"缩放一次，不再被下游技能重复缩放。这个条件只影响"外部强制传基础值"这一种场景，技能自己正常计算伤害的路径（涵盖之前排查过的204个技能等）完全不受影响。

**影响范围**：这个bug是引擎级公式问题，不止影响恐惧魔——所有"用`TRIGGER_SPELL_WITH_VALUE`转发一个自身也带缩放属性的技能"的组合都会中招，具体还有哪些没有逐个排查，本次只针对这一个报告的案例验证。

**是否数据库/代码改动**：纯代码改动（`src/game/Entities/Object.cpp`），需要重新编译部署才能生效。

**状态**：⏳ 代码已改，逻辑推导+手工核算数值吻合，尚未通过实测日志确认（恐惧魔那边还没有部署后的真实战斗记录验证）。
